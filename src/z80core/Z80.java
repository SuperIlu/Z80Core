...
public class Z80 {
	...
	/*
	 * Flags para indicar la modificación del registro F en la instrucción
	 * actual y en la anterior. Son necesarios para emular el comportamiento de
	 * los bits 3 y 5 del registro F con las instrucciones CCF/SCF.
	 * 
	 ...
	 * implementado hasta ahora parece que funciona. El resto de la historia
	 * está contada en: http://zx.pk.ru/attachment.php?attachmentid=2989
	 *
	 * 25/09/2009 Se ha completado la emulación de MEMPTR. A señalar que no se
	 * puede comprobar si MEMPTR se ha emulado bien hasta que no se emula el
	 * comportamiento del registro en las instrucciones CPI y CPD. Sin ello,
	 * todos los tests de z80tests.tap fallarán aunque se haya emulado bien al
	 * registro en TODAS las otras instrucciones. Shit yourself, little parrot.
	 */
	private int memptr;

	/*
	 * Algunos flags se precalculan para un tratamiento más rápido
	 * Concretamente, SIGN, ZERO, los bits 3, 5, PARITY y ADDSUB: sz53n_addTable
	 * tiene el ADDSUB flag a 0 y paridad sin calcular sz53pn_addTable tiene el
	 * ADDSUB flag a 0 y paridad calculada sz53n_subTable tiene el ADDSUB flag a
	 * 1 y paridad sin calcular sz53pn_subTable tiene el ADDSUB flag a 1 y
	 * paridad calculada El resto de bits están a 0 en las cuatro tablas lo que
	 * es importante para muchas operaciones que ponen ciertos flags a 0 por
	 * real decreto. Si lo ponen a 1 por el mismo método basta con hacer un OR
	 * con la máscara correspondiente.
	 */
	 ...
	 * aprovechar el camino más corto aunque tengan un poco más de código (al
	 * menos en bytecodes lo tienen)
	 */
	 ...
	 * reset es poniendo PC, IFF1, IFF2, R e IM0 a 0 y todos los demás
	 * registros a 0xFFFF.
	 * 
	 * 29/05/2011: cuando la CPU recibe alimentación por primera vez, los
	 * registros PC e IR se inicializan a cero y el resto a 0xFF. Si se produce
	 * un reset a través de la patilla correspondiente, los registros PC e IR
	 * se inicializan a 0 y el resto se preservan. En cualquier caso, todo
	 * parece depender bastante del modelo concreto de Z80, así que se escoge
	 * el comportamiento del modelo Zilog Z8400APS. Z80A CPU.
	...
	private void cp(int oper8) {
		...
		sz5h3pnFlags = (sz53n_addTable[oper8] & FLAG_53_MASK) | // No necesito
																// preservar H,
																// pero está a
																// 0 en la tabla
																// de todas
	...
	// Interrupción
	/*
	 * Desglose de la interrupción, según el modo: IM0: M1: 7 T-Estados ->
	 * reconocer INT y decSP M2: 3 T-Estados -> escribir byte alto y decSP M3: 3
	 * T-Estados -> escribir byte bajo y salto a N IM1: M1: 7 T-Estados ->
	 * reconocer INT y decSP M2: 3 T-Estados -> escribir byte alto PC y decSP
	 * M3: 3 T-Estados -> escribir byte bajo PC y PC=0x0038 IM2: M1: 7 T-Estados
	 * -> reconocer INT y decSP M2: 3 T-Estados -> escribir byte alto y decSP
	 * M3: 3 T-Estados -> escribir byte bajo M4: 3 T-Estados -> leer byte bajo
	 * del vector de INT M5: 3 T-Estados -> leer byte alto y saltar a la rutina
	 * de INT
	 */
	...
	/*
	 * Desglose de ciclos de máquina y T-Estados M1: 5 T-Estados -> extraer
	 * opcode (pá ná, es tontería) y decSP M2: 3 T-Estados -> escribe byte
	 * alto de PC y decSP M3: 3 T-Estados -> escribe byte bajo de PC y PC=0x0066
	 */
	private void nmi() {
	 ...
	 * ciclos de máquina reales que se ejecutan. Esa es la única forma de
	 * poder simular la contended memory del Spectrum.
	 */
	...
	private void decodeCB() {
		...
		default: {
			// System.out.println("Error instrucción CB " +
	 ...
	 * activan IY con el primer FD, IX con el segundo DD y vuelven al registro
	 * HL con el código NOP. Es decir, si detrás del código DD/FD no viene
	 * una instrucción que maneje el registro HL, el código DD/FD "se olvida"
	 * y hay que procesar la instrucción como si nunca se hubiera visto el
	 * prefijo (salvo por los 4 t-estados que ha costado). Naturalmente, en una
	 * serie repetida de DDFD no hay que comprobar las interrupciones entre cada
	 * prefijo.
	 */
	private int decodeDDFD(int regIXY) {
		...
		default: {
			...
			// System.out.println("Error instrucción DD/FD" +
			// Integer.toHexString(opCode));
	...
	private void decodeED() {
		...
		case 0x47: { /* LD I,A */
			/*
			 * El contended-tstate se produce con el contenido de I *antes* de
			 * ser copiado el del registro A. Detalle importante.
			 */
			Z80opsImpl.contendedStates(getPairIR(), 1);
			regI = regA;
			break;
		...
		default: {
			// System.out.println("Error instrucción ED " +
			// Integer.toHexString(opCode));
			break;
		}
		}
